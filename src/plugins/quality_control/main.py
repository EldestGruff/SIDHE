...
class QualityControlPlugin:
    ...
    async def run_full_quality_check(self, scope: str = "all") -> QualityReport:
        ...
        try:
            ...
            linting_results, coverage_results, metrics_results = await asyncio.gather(*tasks)
            ...
            issues = self._compile_issues(linting_results, coverage_results, metrics_results)
            recommendations = await self._generate_recommendations(issues, overall_score)
            
            # Evaluate quality gates
            gate_status = await self._evaluate_quality_gates(overall_score, coverage_results)
            ...
            logger.info(f"Quality check completed in {duration:.2f}s (score: {overall_score:.1f})")
    ...
    async def get_quality_status(self) -> QualityStatus:
        ...
        return QualityStatus(
            ...
            overall_score=self.current_status.overall_score if self.current_status else None,
    ...
    async def enforce_quality_gates(self, quest_id: str) -> Dict[str, Any]:
        ...
        gates = {
            ...
            "linting_compliance": self._check_linting_compliance(quality_report.linting_results),
            "coverage_threshold": self._check_coverage_threshold(quality_report.coverage_results),
            "no_critical_issues": len([i for i in quality_report.issues if i.get("severity") == "critical"]) == 0
        }
        ...
        result = {
            ...
            "blocking_issues": [i for i in quality_report.issues if i.get("severity") in ["critical", "high"]],
            "timestamp": datetime.now().isoformat()
        }
        
        logger.info(f"Quality gates for {quest_id}: {'PASSED' if all_gates_passed else 'FAILED'}")
        return result
    ...
    async def _get_all_source_files(self) -> List[str]:
        ...
        # Python files
        for pattern in ["**/*.py"]:
            files.extend([str(f) for f in project_root.glob(pattern) if not self._should_ignore_file(f)])
    ...
    async def _get_backend_files(self) -> List[str]:
        ...
        backend_path = Path(__file__).parent.parent.parent / "conversation_engine" / "backend"
        return [str(f) for f in backend_path.glob("**/*.py") if not self._should_ignore_file(f)]
    
    async def _get_frontend_files(self) -> List[str]:
        """Get frontend source files"""
        frontend_path = Path(__file__).parent.parent.parent / "conversation_engine" / "frontend"
        files = []
        for pattern in ["**/*.js", "**/*.jsx", "**/*.ts", "**/*.tsx"]:
            files.extend([str(f) for f in frontend_path.glob(pattern) if not self._should_ignore_file(f)])
        return files
    
    async def _get_plugin_files(self) -> List[str]:
        """Get plugin source files"""
        plugins_path = Path(__file__).parent.parent
        return [str(f) for f in plugins_path.glob("**/*.py") if not self._should_ignore_file(f)]
    ...
    async def _run_linting_checks(self, files: List[str]) -> Dict[str, Any]:
        ...
        if tasks:
            ...
            if js_files:
                results['javascript'] = lint_results[-1] if js_files else lint_results[1] if len(lint_results) > 1 else {}
    ...
    def _compile_issues(self, linting_results: Dict, coverage_results: Dict, metrics_results: Dict) -> List[Dict[str, Any]]:
    ...
    async def _generate_recommendations(self, issues: List[Dict], overall_score: float) -> List[str]:
        """Generate actionable recommendations based on issues"""
        recommendations = []
        
        # Score-based recommendations
        if overall_score < 60:
            recommendations.append("Critical: Overall quality score is below acceptable threshold. Immediate attention required.")
        elif overall_score < 80:
            recommendations.append("Warning: Quality score needs improvement. Focus on high-impact issues.")
        ...
        if issue_types.get('linting', 0) > 10:
            recommendations.append("High number of linting issues. Run automated formatting tools.")
        
        if issue_types.get('coverage', 0) > 0:
            recommendations.append("Test coverage is below target. Add tests for uncovered code paths.")
        
        if issue_types.get('quality', 0) > 5:
            recommendations.append("Code complexity issues detected. Consider refactoring complex functions.")
        
        return recommendations
    
    async def _evaluate_quality_gates(self, overall_score: float, coverage_results: Dict) -> Dict[str, bool]:
        ...
        return {
            ...
            "critical_issues": len([i for i in self.current_status.issues if i.get('severity') == 'critical']) == 0 if self.current_status else True
        }

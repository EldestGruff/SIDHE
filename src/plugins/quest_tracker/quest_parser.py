...
class QuestParser:
    ...
    def _extract_quest_id(self, body: str) -> str:
        """Extract quest ID from body"""
        # Look for patterns like "Quest ID: QUEST-001" or "**Quest ID:** QUEST-001"
        patterns = [
            r'\*\*Quest ID:\*\*\s*([A-Z]+-\d+)',
            r'Quest ID:\s*([A-Z]+-\d+)',
            r'Quest ID:\s*`([A-Z]+-\d+)`',
    ...
    def _extract_quest_type(self, body: str) -> str:
        """Extract quest type from body"""
        # Look for patterns like "Type: Implementation" or "Quest Type: Bug Fix"
        patterns = [
            r'\*\*(?:Quest\s+)?Type:\*\*\s*([^\n]+)',
            r'(?:Quest\s+)?Type:\s*([^\n]+)',
            r'Category:\s*([^\n]+)'
    ...
    def extract_objectives(self, body: str) -> List[MissionObjective]:
        ...
        objectives_match = re.search(r'#{1,4}\s*🎯\s*Quest\s+Objectives(.*?)(?=#{1,4}|$)', body, re.DOTALL | re.IGNORECASE)
    ...
    def _extract_captains_notes(self, body: str) -> Optional[str]:
        """Extract archmage's notes section"""
        # Look for archmage's notes, notes, or comments section
        patterns = [
            r'#{1,4}\s*👨‍✈️\s*Archmage[\'\']?s?\s+Notes?(.*?)(?=#{1,4}|$)',
            r'#{1,4}\s*Archmage[\'\']?s?\s+Notes?(.*?)(?=#{1,4}|$)',
            r'#{1,4}\s*Notes?(.*?)(?=#{1,4}|$)',
    ...
    def extract_referenced_docs(self, body: str) -> List[str]:
        ...
        # Look for grimoire references
        crew_refs = re.findall(r'grimoire/([^\s]+\.md)', body)
        docs.extend([f"grimoire/{ref}" for ref in crew_refs])

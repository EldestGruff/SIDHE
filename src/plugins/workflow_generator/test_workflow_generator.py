...
class TestTemplateLibrary:
    ...
    def test_init_loads_default_templates(self):
        ...
        assert "react-app-deploy" in library.templates
        assert "git-workflow" in library.templates
        assert "docker-build-deploy" in library.templates
    ...
    def test_find_matches_partial_keywords(self):
        ...
        assert len(matches) > 0
        # Should find react-app-deploy template
        react_matches = [m for m in matches if m.template.name == "react-app-deploy"]
...
class TestRedisWorkflowStore:
    ...
    def test_list_workflows(self):
        ...
        with patch('redis.from_url') as mock_redis:
            ...
            # Mock workflow keys
            workflow_keys = ["riker:workflow:test1:1.0", "riker:workflow:test2:1.0"]
    ...
    def test_cleanup_expired_data(self):
        ...
        with patch('redis.from_url') as mock_redis:
            ...
            mock_redis_client.smembers.side_effect = [
                ["riker:workflow:test1:1.0", "riker:workflow:test2:1.0"],  # workflows
                ["riker:execution:exec1", "riker:execution:exec2"]  # executions
            ]

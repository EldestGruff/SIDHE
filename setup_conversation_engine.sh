...
# Creates structure and guides artifact deployment for when you're not feeling sharp
# 
# Usage: bash setup_conversation_engine.sh
...
if [[ ! -d ".git" ]] || [[ ! -f "README.md" ]] || [[ ! -d "src" ]]; then
    print_error "This doesn't look like the root of your Riker project."
    echo "Please run this script from the Riker project root directory."
    exit 1
fi

echo "🚀 Riker Conversation Engine Foundation Setup"
echo "============================================="
echo "Setting up foundation structure and guiding artifact deployment..."
echo ""

# Step 1: Create directory structure
print_step "1" "Creating complete directory structure..."

mkdir -p src/conversation_engine/{docs,docker}
mkdir -p src/conversation_engine/backend/{config,websocket,intent,bus,memory,plugins}
mkdir -p src/conversation_engine/frontend/{public,src/{components/{Chat,Dashboard},hooks,services}}
mkdir -p src/conversation_engine/tests/{backend,frontend,integration}
...
# Step 2: Create artifact deployment guide
print_step "2" "Creating artifact deployment guide..."

cat > src/conversation_engine/ARTIFACT_DEPLOYMENT.md << 'EOF'
...
cat > src/conversation_engine/backend/bus/publisher.py << 'EOF'
"""
...
cat > src/conversation_engine/backend/memory/integration.py << 'EOF'
"""
...
cat > src/conversation_engine/backend/plugins/registry.py << 'EOF'
"""
...
<html lang="en">
  <head>
    ...
    <title>Riker Conversation Engine</title>
  </head>
  <body>
...
cat > src/conversation_engine/frontend/src/App.css << 'EOF'
.App {
  text-align: center;
}
...
if [[ ! -f "src/conversation_engine/.env" ]]; then
    cp src/conversation_engine/.env.example src/conversation_engine/.env
...
cat > src/conversation_engine/verify_foundation.sh << 'EOF'
#!/bin/bash
...
chmod +x src/conversation_engine/verify_foundation.sh
...
echo "📋 Next Steps:"
echo ""
print_step "A" "Copy all 20 artifacts using the deployment guide:"
echo "    📄 src/conversation_engine/ARTIFACT_DEPLOYMENT.md"
echo ""
print_step "B" "Verify foundation after copying artifacts:"
echo "    🔍 bash src/conversation_engine/verify_foundation.sh"
echo ""
print_step "C" "Update environment configuration:"
echo "    📝 src/conversation_engine/.env"
echo ""
print_step "D" "Install dependencies (optional):"
echo "    🐍 cd src/conversation_engine/backend && pip install -r requirements.txt"
echo "    📦 cd src/conversation_engine/frontend && npm install"
echo ""
print_step "E" "Create Away Mission for Claude Code:"
echo "    🚀 Use template: .github/ISSUE_TEMPLATE/conversation-engine-mission.md"
echo ""
echo "📖 Documentation:"
echo "   • Specification: crew-quarters/conversation-engine-spec.md"
echo "   • Handoff Guide: src/conversation_engine/docs/claude-handoff.md"
echo "   • Deployment Guide: src/conversation_engine/ARTIFACT_DEPLOYMENT.md"
echo ""
echo "🖖 Foundation structure ready - just copy the artifacts and verify!"
echo "   Run: bash src/conversation_engine/verify_foundation.sh"
